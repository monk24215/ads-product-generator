...
import { validateSlug, validateVendorId, deriveIdentity, buildProductJson, github, b64, clean } from '../lib/template.js';
...
const slug = clean(args[0]).toLowerCase();
const vendor_id = clean(args[1]).toUpperCase();
const product_name = clean(args[2]);
...
try {
  ...
  await github(GITHUB_TOKEN, 'POST', `/repos/${templateOwner}/${TEMPLATE_REPO}/generate`, {
    ...
    description: clean(`${product_name} - sales page service`),
    private: true,
  ...
  try {
    const r = await fetch(`${ADS_ACCESS_BASE}/api/catalog`, {
      ...
      body: JSON.stringify({ slug, title: product_name, books: [] }),
    });
    if (!r.ok) {
